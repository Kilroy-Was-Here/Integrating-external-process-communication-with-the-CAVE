...
#Downloading from Xeric server
with open("cloud_auth.txt") as f:
	auth = f.readline().rstrip()

username, password = auth.split(":")

pixel_width=8192
#pixel_width=2048    #Downloading 2048 does not work

remote_path = "https://secure.xericdesign.com/xplanet/clouds/{}".format(pixel_width)
remote_file = "clouds-{}.jpg".format(pixel_width)
...
earthCloudsTexture = viz.addBlankTexture(size=img.size, type=viz.TEX_2D, format=viz.TEX_RGBA)
earthCloudsTexture.setImageData(data=img.tobytes(), size=img.size, format=viz.TEX_RGBA)
